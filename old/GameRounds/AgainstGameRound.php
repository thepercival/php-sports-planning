 ...
 */
final class AgainstGameRound extends ListNode
{
    use GameRoundTrait;
    ...
    public function __construct(
        ...
        AgainstGameRound|null $previous = null)
    {
    ...
    public function add(HomeAway $homeAway): void
    {
        ...
        foreach ($homeAway->getPlaceNrs() as $placeNr) {
            $this->placeMap[(string)$place] = $placeNr;
        }
    }
    ...
    public function remove(HomeAway $homeAway): void
    {
        ...
        foreach ($homeAway->getPlaceNrs() as $placeNr) {
            unset($this->placeMap[(string)$this->poule->getPlace($placeNr)]);
        }
    }
    ...
    {
        foreach ($homeAway->getPlaceNrs() as $placeNr) {
            if ($this->isParticipating($this->poule->getPlace($placeNr))) {
                return true;
            }
        }
        return false;
