...
final class PlaceNrCombinationCounterMap
{
    /**
     * @var array<string, PlaceNrCombinationCounter>
     */
    private array $map;
    ...
    /**
     * @var array<int, list<PlaceNrCombinationCounter>>
     */
    private array|null $perAmount = null;

    /**
     * @param array<string, PlaceNrCombinationCounter> $placeNrCombinationCounters
     */
    public function __construct(array $placeNrCombinationCounters)
    {
        $this->map = $placeNrCombinationCounters;
    }

    public function getPlaceNrCombination(string $index): PlaceNrCombination
    {
        return $this->map[$index]->getPlaceNrCombination();
    }

    public function count(PlaceNrCombination $placeNrCombination): int
    {
        if( !array_key_exists($placeNrCombination->getIndex(), $this->map) ) {
            return 0;
        }
        return $this->map[$placeNrCombination->getIndex()]->count();
    }

    /**
     * @return list<PlaceNrCombinationCounter>
     */
    public function getPlaceNrCombinationCounters(): array
    {
    ...
    public function addPlaceNrCombination(PlaceNrCombination $placeNrCombination): self {
        ...
        $newCounter = $this->map[$placeNrCombination->getIndex()]->increment2();
        $map = $this->map;
        $map[$placeNrCombination->getIndex()] = $newCounter;
    ...
    public function removePlaceNrCombination(PlaceNrCombination $placeNrCombination): self {
        ...
        $newCounter = $this->map[$placeNrCombination->getIndex()]->decrement();
        $map = $this->map;
        $map[$placeNrCombination->getIndex()] = $newCounter;
...
//    }
    ...
    protected function canBeBalanced(): bool {
        if( $this->canBeBalanced === null) {
            $totalCount = 0;
            foreach( $this->map as $placeNrCombinationCounter) {
                $totalCount += $placeNrCombinationCounter->count();
            }
    ...
    /**
     * @return array<int, list<PlaceNrCombinationCounter>>
     */
    ...
    public function output(LoggerInterface $logger, string $prefix, string $header): void {
        ...
        foreach( $this->getPlaceNrCombinationCounters() as $counterIt ) {
    ...
    {
        $map = [];
        foreach( $this->map as $index => $placeNrCombinationCounter) {
            $map[$index] = clone $placeNrCombinationCounter;
        }
        $this->map = $map;
    }
}
